...
def log_batch_errors(batch_errors_list, log_writer, namespace, curr_iter, bar_plot=[], box_plot=[]):
    ...
    for metric in batch_errors_list[0].keys():
        metric_batch_error = []
        for batch_errors in batch_errors_list:
            metric_batch_error.extend(batch_errors[metric])

        if len(metric_batch_error) > 0:
            log_writer.add_histogram(f"{namespace}/{metric}", metric_batch_error, curr_iter)
            log_writer.add_scalar(f"{namespace}/{metric}_mean", np.mean(metric_batch_error), curr_iter)
            log_writer.add_scalar(f"{namespace}/{metric}_median", np.median(metric_batch_error), curr_iter)
            ...
            if metric in bar_plot:
                ...
                evaluate.visualization_utils.plot_barplots(ax, pd, 'dataset', metric)
                log_writer.add_figure(f"{namespace}/{metric}_bar_plot", kde_barplot_fig, curr_iter)

            if metric in box_plot:
                mse_fde_pd = {'dataset': [namespace] * len(metric_batch_error),
                                metric: metric_batch_error}
                fig, ax = plt.subplots(figsize=(5, 5))
                evaluate.visualization_utils.plot_boxplots(ax, mse_fde_pd, 'dataset', metric)
                log_writer.add_figure(f"{namespace}/{metric}_box_plot", fig, curr_iter)
