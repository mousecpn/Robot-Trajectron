...
class TransformerConcatLinear(Module):
    ...
    def __init__(self, point_dim, context_dim, tf_layer, residual):
        ...
        # self.residual = residual
        # self.pos_emb = PositionalEncoding(d_model=context_dim, dropout=0.1, max_len=24)
        # self.concat1 = ConcatSquashLinear(3,context_dim,context_dim+3)
        # self.layer = nn.TransformerEncoderLayer(d_model=context_dim, nhead=4, dim_feedforward=2*context_dim)
        # self.transformer_encoder = nn.TransformerEncoder(self.layer, num_layers=tf_layer)
        # self.concat3 = ConcatSquashLinear(context_dim,context_dim,context_dim+3)
        # self.concat4 = ConcatSquashLinear(context_dim,context_dim//2,context_dim+3)
